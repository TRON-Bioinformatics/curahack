...
### Human ###
# 86
awk -v OFS="\t" '$7 == "+" {print $1, $4, $5, $10, $14}' <(zcat reference/Homo_sapiens.GRCh38.86.chr1.gtf.gz) \
...
# 109
```
awk -v OFS="\t" '$7 == "+" {print $1, $4, $5, $10, $14}' <(zcat reference/Homo_sapiens.GRCh38.109.chr1.gtf.gz) \
| sed 's/"//g' | sed 's/ //g' | sed 's/;//g' \
| grep -hv 'ENST' \
| sort -k 6,6 -k2,2n \
...
### bed to fasta ###
####################
bedtools getfasta -fi reference/Homo_sapiens.GRCh38.dna.primary_assembly.chr1.fa \
 -bed genes_hg38_86_both.bed \
 -fo hg38_86_gtf.fa

bedtools getfasta -fi reference/Homo_sapiens.GRCh38.dna.primary_assembly.chr1.fa \
 -bed genes_hg38_109_both.bed \
 -fo hg38_109_gtf.fa
````
